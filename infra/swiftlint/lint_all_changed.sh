...
CHANGED_SWIFT_FILES_TO_MASTER=$(git diff $(git merge-base --fork-point master) --diff-filter=d --name-only | grep ".swift$" | xargs -I '{}' realpath --relative-to=. $(git rev-parse --show-toplevel)/'{}')
...
$SWIFT_LINT lint --quiet --use-alternative-excluding --use-script-input-files --force-exclude $@
